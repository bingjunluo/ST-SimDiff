...
class SigLipVisionTower(nn.Module):
    def __init__(self, vision_tower, vision_tower_cfg, tok_keep_ratio=None,delay_load=False,dominant_num = None,contextual_num =None):
        super().__init__()
        ...
        self.dominant_num=dominant_num
        self.contextual_num=contextual_num
    ...
    def feature_select(self, image_forward_outs):
        # FasterVLM: select the most important tokens based on [cls] attention
        image_features = image_forward_outs.hidden_states[self.select_layer] # (batch_size, num_tokens + 1, embedding_dim)
        # 1. Obtain attention matrix
        image_attentions = image_forward_outs.attentions[self.select_layer] # (batch_size, num_heads, num_tokens + 1, num_tokens + 1)

        # 2. Obtain [cls] attention
        cls_attention = torch.mean(image_attentions[:, :, 0, 1:], dim=1) # (batch_size, num_tokens, num_tokens)
        num_tokens = image_attentions.shape[-1] # (num_tokens,)
        k = math.ceil(num_tokens * self.tok_keep_ratio) # number of tokens retaining
        # 3. Obtain token indices with highest attention scores assigned by [cls] token 
        # and keep the relative position via sorting
        indices = torch.topk(cls_attention, k=k, dim=-1).indices
        indices = torch.sort(indices, dim=-1).values # (batch_size, k)
        # 4. Select the most important tokens
        embedding_dim = image_features.shape[-1] # (embedding_dim,)
        indices = indices.unsqueeze(-1).expand(-1, -1, embedding_dim) # (batch_size, k, embedding_dim)
        image_features = torch.gather(image_features, dim=-2, index=indices)
        # print(image_features.shape) # (batch_size, k, embedding_imd)
    ...
    def vision_zip(self,image_forward_outs,dominant_num = 54,contextual_num =10):
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            if self.tok_keep_ratio!=None:
                image_features = self.feature_select(image_forward_outs).to(images.dtype)
            elif self.dominant_num!=None:
                image_features = self.vision_zip(image_forward_outs,self.dominant_num,self.contextual_num).to(images.dtype)
