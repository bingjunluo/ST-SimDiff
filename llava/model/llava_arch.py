...
class LlavaMetaModel:
    ...
    def __init__(self, config,tok_keep_ratio=None,dominant_num = None,contextual_num =None):
        super(LlavaMetaModel, self).__init__(config)

        if hasattr(config, "mm_vision_tower"):
            delay_load = getattr(config, "delay_load", False)
            self.vision_tower = build_vision_tower(config, tok_keep_ratio=tok_keep_ratio,delay_load=delay_load,dominant_num=dominant_num,contextual_num=contextual_num)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_2dPool(self, image_feature, stride=2):
        ...
        if self.config.mm_spatial_pool_mode == "average":
            ...
            #image_feature = nn.functional.avg_pool2d(image_feature, stride)
            # pool=SoftmaxWeightedL2NormPool2d(kernel_size=(2,2),temp=0,p=2,stride=(2,2),padding=0)
            # image_feature=pool(image_feature)
            height, width = image_feature.shape[2:]
            scaled_shape = [math.ceil(height / stride), math.ceil(width / stride)]
            image_feature = nn.functional.interpolate(image_feature, size=scaled_shape, mode='bilinear')
        elif self.config.mm_spatial_pool_mode == "l2norm":
            pool=SoftmaxWeightedL2NormPool2d(kernel_size=(self.config.kernal_size,self.config.kernal_size),temp=self.config.temp,stride=(2,2),padding=self.config.padding)
            image_feature=pool(image_feature)
        else:
